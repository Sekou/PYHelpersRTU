...
shift, scale, eps = [0,0], 50, 0.0000001 
def getT(): return np.array([[scale,0,0],[0,scale,0],[shift[0],shift[1],1]])
def EXT(v): return [*v,1]
def CUT(v): return v[:-1] # def trP(p): return CUT(getT()@EXT(p))
...
class Draw:
    ...
    def cell(self, screen, color, pos, sz, w=1):  # отрисовка квадратной дискреты
        eps, bb = w/scale, (pos[0] - sz / 2, pos[1] - sz / 2, sz+eps, sz+eps)
