...
def bb_contains(bb, p): return bb[0] < p[0] < bb[0] + bb[2] and bb[1] < p[1] < bb[1] + bb[3]
...
def manip_ik(lens, angs, lims, p0, ang0, target, step=0.1, iters=1): #решение ОЗК для многозвенного 2d-манипулятора
    ...
    for i in range(len(angs)-1, -1, -1): # легче вращ. последн. звено - начинаем с него
        ...
        while dir!=0: #пробегаемся сначала в плюс; если ошибка растет, то в минус
            A[i], e = min(lims[i][1], max(lims[i][0], A[i]+dir*step)), E(manip_fk(lens, A, p0, ang0) - tg)
            if e < e_min: e_min, a_best = e, A[i]
            if e>e0 or e==e_last: A[i], dir=A[i]-(1.5+0.5*dir)*step, (-1 if dir==1 else 0)
            e_last = e
        A[i]=a_best
...
class TaskGoTo(Task): #задача движения к точке #t=TaskGoTo((100,100))
    def __init__(self, goal): _, self.goal = super().__init__(), goal
...
def find_route_dijkstra(nodes, n1, n2): #поиск маршрута на графе при помощи алдгоритма Дейкстры
	for n in nodes: n.visited, n.D, n.route = False, 100500, []
	n1.D, n1.route, wave = 0, [n1], [n1]
	while len(wave): # requires class hierarchy: Graph -> Node -> Edge
