...
def gen_surface_points(n):
    inps=np.meshgrid(2*np.arange(n)/(n-1)-1, 2*np.arange(n)/(n-1)-1)
    return np.moveaxis(np.array([*inps, F(*inps)]), 0, -1).reshape((n*n,3))

def draw_pts(points):
    glPointSize(5.0), glColor((1, 1, 1)), glBegin(GL_POINTS)
